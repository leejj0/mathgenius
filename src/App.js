        ...
        {!loading && error && (
          <div className="empty-area">
            <svg width="60" height="60" viewBox="0 0 60 60" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <ellipse cx="22" cy="28" rx="4" ry="6" fill="#fff" />
              <ellipse cx="38" cy="28" rx="4" ry="6" fill="#fff" />
              <ellipse cx="22" cy="32" rx="2" ry="2.5" fill="#636e72" />
              <ellipse cx="38" cy="32" rx="2" ry="2.5" fill="#636e72" />
              <path d="M20 44 Q30 36 40 44" stroke="#e17055" strokeWidth="3" fill="none" />
            </svg>
            <div className="empty-text">生成失败，请稍后重试！</div>
          </div>
        )}
        {!loading && !error && (!questionData || questionData.length === 0) && (
          <div className="empty-area">
            <svg width="60" height="60" viewBox="0 0 60 60" fill="none" xmlns="http://www.w3.org/2000/svg">
              <circle cx="30" cy="30" r="28" fill="#dff9fb" stroke="#00b894" strokeWidth="2" />
              <ellipse cx="22" cy="28" rx="4" ry="6" fill="#fff" />
              <ellipse cx="38" cy="28" rx="4" ry="6" fill="#fff" />
              <ellipse cx="22" cy="28" rx="2" ry="3" fill="#636e72" />
              <ellipse cx="38" cy="28" rx="2" ry="3" fill="#636e72" />
              <path d="M20 40 Q30 48 40 40" stroke="#00b894" strokeWidth="3" fill="none" />
            </svg>
            <div className="empty-text">快来生成你的专属数学题吧！</div>
          </div>
        )}
        {!loading && !error && questionData && questionData.length > 0 && (
          <div>
            <QuestionCard {...questionData[currentIndex]} index={currentIndex + 1} />
            <div style={{ marginTop: 16, display: 'flex', justifyContent: 'center', gap: 16 }}>
              <button
                onClick={() => setCurrentIndex(i => i - 1)}
                disabled={currentIndex === 0}
                style={{ padding: '8px 18px', borderRadius: 12, fontSize: '1rem', opacity: currentIndex === 0 ? 0.5 : 1 }}
              >
                上一题
              </button>
              <span style={{ alignSelf: 'center', fontWeight: 'bold', color: '#00b894' }}>
                {currentIndex + 1} / {questionData.length}
              </span>
              <button
                onClick={() => setCurrentIndex(i => i + 1)}
                disabled={currentIndex === questionData.length - 1}
                style={{ padding: '8px 18px', borderRadius: 12, fontSize: '1rem', opacity: currentIndex === questionData.length - 1 ? 0.5 : 1 }}
              >
                下一题
              </button>
            </div>
